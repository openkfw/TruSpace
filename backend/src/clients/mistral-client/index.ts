...
export class MistralClient implements BackendLLMClient {
  ...
  async generateCompletion(
    ...
    try {
      ...
      const chatCompletion = await this.mistral.chat.complete({
  ...
  async #detectLanguage({
    ...
    try {
      ...
      const content = chatCompletion.choices[0].message.content;
      const language = Array.isArray(content)
        ? content
            .filter((c) => c.type === "text")
            .map((c) => c.text)
            .join("")
  ...
  async #generateTags({
    ...
    try {
      ...
      const tagsString = Array.isArray(content)
        ? content
            .filter((c) => c.type === "text")
            .map((c) => c.text)
            .join("")
  ...
  async #generatePerspectives({
    ...
    try {
      ...
      for (const prompt of prompts) {
        const mistralResponse = await this.generateCompletion(
          fileData,
          prompt.prompt,
          prompt.title
        );
  ...
  async uploadFile(file: UploadedFile): Promise<FileData | { error: string }> {
    try {
      ...
      logger.error(
        `Error processing file in MistralClient.uploadFile: ${error.message}`
      );
      return {
        error:
