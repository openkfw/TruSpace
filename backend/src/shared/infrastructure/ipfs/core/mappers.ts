...
// Tag names are stored URI-encoded (see postTag / #generateTags). Decode them here so every
// consumer (search, filtering, display) works with the same plain-text representation,
// regardless of whether the tag was created by a human or generated by the AI.
function decodeTagName(name: string | undefined): string {
  if (!name) return '';
  try {
...
export function transformPinToTag(pin: Pin): Tag {
  return {
    ...
    meta: {
      ...
      versionCid: pin.meta.versionCid,
      timestamp: pin.meta.timestamp,
      name: decodeTagName(pin.meta.name),
